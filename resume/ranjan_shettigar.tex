...
% \singlespacing
\onehalfspacing % \onehalfspacing sets line spacing to 1.5x normal between lines in a paragraph.
% \setlength{\parskip}{0.6em} only adds space between paragraphs, not lines.
% The quoted text is a single paragraph, so only 1.5x line spacing applies between lines.
...
\newcommand{\entrytitle}[4]{%
    {\fontsize{12pt}{14pt}\selectfont\textbf{#1 | #2 | #3 | #4}}\\[\parskip] % 12pt for job/project titles
}
\newcommand{\projecttitle}[2]{%
    {\fontsize{12pt}{14pt}\selectfont\textbf{#1 | #2}}\\[\parskip] % 12pt for project titles
}

% --- Define body text font size macro ---
\newcommand{\bodyfont}{\fontsize{10pt}{12pt}\selectfont\setstretch{1.5}} % 10pt for body text
...
% --- Header ---
\begin{center}
    {\fontsize{18pt}{22pt}\selectfont\bfseries RANJAN SHETTIGAR} % 18pt for main header
    \\[1.2ex]
